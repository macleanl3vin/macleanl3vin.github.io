...
/* --------------------------------------------------------------------------
 * Compartmental structure schematic
 * ----------------------------------------------------------------------- */

export function CompartmentDiagram({ className = "" }: { className?: string }) {
  const cyan = accentVar.cyan;
  const teal = accentVar.teal;
  ...
  return (
    <svg
      ...
      className={`h-auto w-full ${className}`}
      role="img"
      aria-label="Compartmental model: an oral dose enters a depot compartment, is absorbed at rate k a into a central plasma compartment of volume V, and is eliminated by saturable metabolism."
    >
      <defs>
        <marker
          id="comp-arrow"
      ...
      <g fill="none" strokeWidth="1.6" opacity="0.8">
        <path d="M44 110 L 130 110" stroke={teal} color={teal} markerEnd="url(#comp-arrow)" />
        <path d="M282 110 L 386 110" stroke={cyan} color={cyan} markerEnd="url(#comp-arrow)" />
        <path d="M538 110 L 648 110" stroke={cyan} color={cyan} markerEnd="url(#comp-arrow)" />
      </g>
      ...
      <text
        ...
        textAnchor="middle"
        fill="var(--color-faint)"
        style={{ fontFamily: "var(--font-mono)", fontSize: 14, letterSpacing: "0.1em" }}
      >
        DEPOT
      </text>

      <RateLabel x={334} y={96} text="ka" />
      <RateLabel x={593} y={96} text="Vmax / Km" />

      <NodeBox x={130} y={84} w={152} h={54} label="A_depot" sub="ABSORPTION SITE" color={teal} />
      <NodeBox x={386} y={84} w={152} h={54} label="C · V" sub="CENTRAL / PLASMA" color={cyan} emphasis />
      <NodeBox x={648} y={84} w={82} h={54} label="CL" sub="CLEARED" color={cyan} />
      ...
      <text
        ...
        style={{ fontFamily: "var(--font-mono)", fontSize: 16, letterSpacing: "0.1em" }}
      >
        F·D
      </text>
      <text
...
function RateLabel({ x, y, text }: { x: number; y: number; text: string }) {
  return (
    <text
      x={x}
      y={y}
      textAnchor="middle"
      fill="var(--color-muted)"
      style={{ fontFamily: "var(--font-mono)", fontSize: 15, letterSpacing: "0.06em" }}
    >
      {text}
    </text>
  );
